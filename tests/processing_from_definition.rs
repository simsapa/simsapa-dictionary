...
fn grammar_note() {
    ...
    let sources: Vec<PathBuf> = vec![PathBuf::from("./tests/data/grammar_note/grammar_note.md")];
    ...
    let res = ebook.entries_as_markdown();
    //let mut file = File::create(&PathBuf::from("./tests/data/grammar_note/grammar_note_expect.md")).unwrap();
    //file.write_all(res.as_bytes()).unwrap();
    //let expect = "".to_string();

    let expect = fs::read_to_string(&PathBuf::from("./tests/data/grammar_note/grammar_note_expect.md")).unwrap();
